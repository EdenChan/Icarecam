...
class Goods extends Admin_Controller{
	...
	public function index($offset = ''){
		...
		$this->load->view('goods_list.php',$data);
	}
	...
	public function add(){
		...
		$this->load->view('goods_add.php',$data);
	}
	...
	public function insert(){
		...
		$data['goods_params'] = $this->input->post('goods_params');
	...
	public function update(){
		...
		# 进行更新
			...
			$data['add_time'] = date("Y-m-d H:i:s");
				...
				if ($goods_id = $this->goods_model->update_goods($data,$goods_id)) {
					# 添加商品成功,获取属性并插入到商品属性关联表中
					// $attr_ids = $this->input->post('attr_id_list');
					// $attr_values = $this->input->post('attr_value_list');
					// foreach ($attr_values as $k => $v) {
