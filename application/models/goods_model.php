...
class Goods_model extends CI_Model
{
    ...
    {
        $query = $this->db->where('is_index', 1)->limit(2, 0)->get(self::TBL_GOODS);
        return $query->result_array();
    }
    ...
    {
        $condition['goods_id'] = $goods_id;
        $query = $this->db->where($condition)->delete(self::TBL_GOODS);
    ...
    #更新
    public function update_goods($data, $goods_id)
    {
        $condition['goods_id'] = $goods_id;
        $query = $this->db->where($condition)->update(self::TBL_GOODS, $data);
        return $query ? $goods_id : false;
    }
